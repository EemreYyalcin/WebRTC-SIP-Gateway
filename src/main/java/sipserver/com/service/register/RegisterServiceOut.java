...
public class RegisterServiceOut extends Service {
	...
	public void processResponse(ResponseEvent responseEvent, SipServerTransport transport) {
		try {
			...
			ServerCore.getServerCore().getTrunkExtension(trunkExtension.getExten()).isRegisterResponseRecieved = true;
			ServerCore.getServerCore().getTrunkExtension(trunkExtension.getExten()).registerResponseCode = statusCode;
	...
	public void register(Extension extTrunk) {
		try {
			...
			extTrunk.isRegisterResponseRecieved = false;
			...
			if (!extTrunk.isRegisterResponseRecieved) {
				LogTest.log(extTrunk, "RegisterOut Timeout Message");
				return;
			}

			if (extTrunk.registerResponseCode == 911) {
				LogTest.log(extTrunk, "RegisterOut Response Code Fail ");
				return;
			}
			if (extTrunk.registerResponseCode != SipServerSharedProperties.errorResponseCode && extTrunk.registerResponseCode == Response.UNAUTHORIZED) {
				extTrunk.isRegisterResponseRecieved = false;
			...
			if (extTrunk.registerResponseCode != 200) {
				LogTest.log(extTrunk, "RegisterOut Register Error " + extTrunk.registerResponseCode);
				return;
			}
